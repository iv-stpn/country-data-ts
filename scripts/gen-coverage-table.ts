...
const phoneByCode = new Map(COUNTRY_PHONE_DATA.map((c) => [c.code as string, c]));
...
function callingCodeCell(code: string): string {
...
function phoneMaskCell(code: string): string {
  const cfg = phoneByCode.get(code);
  if (!cfg) return "❌";
...
function phoneExampleCell(code: string): string {
  const cfg = phoneByCode.get(code);
  if (!cfg) return "❌";
  return `✅ ${cfg.example}`;
}
